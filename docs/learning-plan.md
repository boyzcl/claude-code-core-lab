...
如果安装或验证失败，先看 [故障排查（Troubleshooting）](troubleshooting.md)，不要直接跳到历史材料或大型 reference 文档。
...
七节主线学完后，再做 [capstone-mini-runtime](../projects/capstone-mini-runtime/README.md)，把 `Search -> Read -> Edit -> Bash -> Verify` 合成一个端到端最小运行时。
...
这个命令验证的是 capstone 参考解法。你自己的 starter 修复可以先用 `node projects/capstone-mini-runtime/starter/mini-runtime.verify.mjs` 观察初始失败，再对照参考解法。
...
| 想做端到端任务 | [capstone-mini-runtime](../projects/capstone-mini-runtime/README.md) |
...
如果能做到这五件事，就说明你不是只读完了文档，而是真的学会了这套代码智能体核心构建逻辑。
