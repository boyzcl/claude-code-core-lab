...
| Coding Agent / Agent | 代码智能体 | 能读代码、改代码、调用工具、跑验证并处理失败的系统。 |
| Agent Core | 代码智能体核心 / 核心运行时 | 不含完整商业产品外壳，专注模型请求、上下文、工具、计划、状态和评测闭环。 |
| Runtime | 运行时 | 真正保存状态、执行工具、检查权限、调用模型和记录证据的程序层。 |
| Policy | 权限策略 / 安全策略 | 决定什么动作允许、拒绝或需要人工批准。 |
| Prompt | 提示词 | 给模型看的规则文本；它可以指导模型，但不能替代权限策略或工具运行时。 |
| clean-room | 公开学习边界 / 不复制式学习 | 指代码和课程由本项目自己写，不复制官方源码、私有提示词或非公开实现。 |
| verify | 验证脚本 | 可运行的检查，用来证明某个机制确实成立。 |
| fixture | 测试夹具 / 受控样例仓库 | 为了验证机制而准备的小型文件、仓库或输入数据。 |
| evidence | 证据 | 运行结果、验证脚本、记录或报告；证据支持能力声明，但不能夸大范围。 |
| out of scope | 不在当前范围 | 当前阶段明确不承诺、不证明的能力。 |
