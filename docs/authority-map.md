...
| `exercises/README.md` | 练习入口 | yes | Course / Lab / Core 之后的最小动手任务入口，不替代课程正文 |
| `projects/README.md` | 项目实践入口 | yes | 端到端学习项目和 capstone 入口 |
| `solutions/README.md` | 参考解法入口 | yes | 练习和项目完成后的参考答案，不作为当前规则或能力证据替代品 |
...
| `docs/reference/claude-code-capability-coverage-matrix.md` | 官方能力覆盖矩阵 | yes | 官方公开能力、本项目转译、verify 证据和 out-of-scope 边界 |
| `docs/reference/open-source-project-standards.md` | 开源项目规范参考 | yes | 把优秀 GitHub 项目经验转译成本项目协作、参考资料和验证标准 |
| `docs/releases/v0.1-learning-preview.md` | release note | yes | v0.1 learning preview 发布说明，不替代当前状态或验证记录 |
...
| 官方能力覆盖矩阵 | `docs/reference/claude-code-capability-coverage-matrix.md` | reference-authority | open-source | active | `docs/reference/open-source-project-standards.md`, `docs/open-source-boundary.md` | official Claude Code docs, `src/**/*.verify.mjs`, `docs/records/core-run-record.md`, `solutions/capstone-mini-runtime/mini-runtime.verify.mjs` | 对外讲“官方能力如何被本项目教学转译”时以该矩阵为准；矩阵不证明官方实现，只绑定官方公开来源、本项目 Course / Lab / Core、verify 证据和 out-of-scope |
...
| Release notes | `docs/releases/v0.1-learning-preview.md` | release-note | release | released | `CHANGELOG.md`, `CURRENT_STATE.md` | `docs/records/core-run-record.md`, `docs/records/labs-verification-record.md`, `npm run docs:links`, `npm run verify:all` | release note 只浓缩版本内容和边界；当前进度仍以 `CURRENT_STATE.md` 为准，行为证明仍以 verify 和 records 为准 |
| 文档链接检查 | `scripts/check-doc-links.mjs` | automation-check | docs | active | none | `npm run docs:links`, `.github/workflows/verify.yml` | link check 只证明本地 Markdown 内部链接存在；外部链接只记录数量，不证明外部内容仍准确 |
...
- evidence_dirs: `src/**/*verify.mjs`, `solutions/capstone-mini-runtime/mini-runtime.verify.mjs`, `docs/records/core-run-record.md`, `docs/records/labs-verification-record.md`, `docs/records/lab-*-run-record.md`
